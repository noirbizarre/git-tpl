...
[`show`, `completion` and `man`](#show-completion-and-man).
...
Branch on [`error.code`](diagnostics.md), never on `message`.
...
See [the authoring loop](../usage/render.md#the-authoring-loop).
...
[`--deny`](../usage/lint.md#choosing-what-fails) says so.
...
1 when `errors` is 0.
An `--allow`ed finding appears nowhere and is counted nowhere.
...
Documented in [status](../usage/status.md).
...
It has its own, narrower mechanism for the same purpose — see `migrations` under [`update`](#update), below.
...
See [ADR-024](../adr/024-template-migrations.md).
...
See [Migrations](../usage/update.md#migrations).
...
part of a patch that should not be merged unread ([ADR-022](../adr/022-backport-unsubstitutes.md)).

`applyCommand` is the `git am` invocation git-tpl declines to run ([ADR-020](../adr/020-backport-is-a-patch.md)),
...
Documented with the command, at [`git tpl test`](../usage/test.md).
