...
class GitTpl < Formula
  ...
  def install
    ...
    # shell and files the output where that shell looks for it. `shell` is a
    # required positional argument with no flag form (see `CompletionArgs` in
    # src/cli.rs), so this relies on Homebrew's default parameter format, which
    # passes it plainly as a trailing argument. `shell_parameter_format: :none`
    # (used here previously) instead passes no argument at all, only `$SHELL`,
    # which this CLI does not read — that mismatch is what made every
    # `brew install` fail with "the following required arguments were not
    # provided: <SHELL>".
    generate_completions_from_executable(bin/"git-tpl", "completion")
  end
