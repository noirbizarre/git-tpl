...
- Keys matching no question are ignored and reported, exactly as for
  `--answers-from`.
- Precedence, stated once and tested, extending item 2's chain:
...
worktree, questions can be answered from a file (item 2), and diagnostics carry
stable codes.
...
This is downstream of item 2: the fixtures *are* answers files, read by the same
